...
function CIFAR:__init(path, mode, batchSize, zeroCodeFix, hadamardSize)
   ...
   if mode == "train" then
      ...
      self.augLabels = torch.Tensor(10, self.hadamardSize):zero()
      self.augLabelsFix = torch.Tensor(10, self.hadamardSize):zero()
      ...
      if (self.hadamardSize == 63) then
      	   self.augLabels[{ {1, 10} }] = subset.augLabels:t()
      elseif (self.hadamardSize == 31) then
           self.augLabels[{ {1, 10} }] = subset.augLabels_31:t()
      elseif (self.hadamardSize == 127) then
	   self.augLabels[{ {1, 10} }] = subset.augLabels_127:t()
      elseif (self.hadamardSize == 255) then
      	   self.augLabels[{ {1, 10} }] = subset.augLabels_255:t()
      end
      if zeroCodeFix == 0 then
	      if (self.hadamardSize == 63) then
        	   self.augLabelsFix[{ {1, 10} }] = subset.augLabelsFix:t()
	      elseif (self.hadamardSize == 127) then
                   self.augLabelsFix[{ {1, 10} }] = subset.augLabelsFix_31:t()
	      elseif (self.hadamardSize == 127) then
        	   self.augLabelsFix[{ {1, 10} }] = subset.augLabelsFix_127:t()
	      elseif (self.hadamardSize == 255) then
        	   self.augLabelsFix[{ {1, 10} }] = subset.augLabelsFix_255:t()
      	      end
      end

      print(self.augLabels)
...
function CIFAR:SaveAugLabelsFixes(path)
  ...
  --print('self.augLabelsFix size = ', self.augLabelsFix:size())
   ...
   elseif (self.hadamardSize == 255) then
        dataSet1.augLabelsFix_255:copy(self.augLabelsFix:t())
   end
